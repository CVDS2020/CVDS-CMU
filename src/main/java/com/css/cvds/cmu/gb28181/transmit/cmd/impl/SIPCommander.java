package com.css.cvds.cmu.gb28181.transmit.cmd.impl;
...
public class SIPCommander implements ISIPCommander {
     ...
     */
    @Override
    public void playStreamCmd(MediaServerItem mediaServerItem, Device device, String channelId,
        ...
        if (userSetting.isSeniorSdp()) {
            if ("TCP-PASSIVE".equalsIgnoreCase(device.getStreamMode())) {
                content.append("m=video " + mediaServerItem.getPort() + " TCP/RTP/AVP 96 126 125 99 34 98 97\r\n");
            } else if ("TCP-ACTIVE".equalsIgnoreCase(device.getStreamMode())) {
                content.append("m=video " + mediaServerItem.getPort() + " TCP/RTP/AVP 96 126 125 99 34 98 97\r\n");
            } else if ("UDP".equalsIgnoreCase(device.getStreamMode())) {
                content.append("m=video " + mediaServerItem.getPort() + " RTP/AVP 96 126 125 99 34 98 97\r\n");
            }
            ...
            if ("TCP-PASSIVE".equalsIgnoreCase(device.getStreamMode())) {
                content.append("m=video " + mediaServerItem.getPort() + " TCP/RTP/AVP 96 97 98 99\r\n");
            } else if ("TCP-ACTIVE".equalsIgnoreCase(device.getStreamMode())) {
                content.append("m=video " + mediaServerItem.getPort() + " TCP/RTP/AVP 96 97 98 99\r\n");
            } else if ("UDP".equalsIgnoreCase(device.getStreamMode())) {
                content.append("m=video " + mediaServerItem.getPort() + " RTP/AVP 96 97 98 99\r\n");
            }
            content.append("a=recvonly\r\n");
            content.append("a=rtpmap:96 PS/90000\r\n");
            content.append("a=rtpmap:98 H264/90000\r\n");
        ...
        content.append("y=" + mediaServerItem.getSsrc() + "\r\n");// ssrc
        ...
        Request request = headerProvider.createInviteRequest(device, channelId, content.toString(), SipUtils.getNewViaTag(), SipUtils.getNewFromTag(), null, mediaServerItem.getSsrc(), callIdHeader);
        transmitRequest(device.getTransport(), request, (e -> {
            ...
            streamSession.put(device.getDeviceId(), channelId, "play", mediaServerItem.getStream(), mediaServerItem.getSsrc(), mediaServerItem.getId(), response, VideoStreamSessionManager.SessionType.play);
            okEvent.response(e);
        });
    }
