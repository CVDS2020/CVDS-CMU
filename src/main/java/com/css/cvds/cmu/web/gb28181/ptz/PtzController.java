package com.css.cvds.cmu.web.gb28181.ptz;
...
public class PtzController {
	...
	public DeferredResult<String> presetQueryApi(@PathVariable String deviceId, @PathVariable String channelId) {
		...
		result.onTimeout(()->{
			...
			msg.setId(uuid);
			msg.setKey(key);
			msg.setData(WVPResult.fail("获取设备预置位超时"));
		...
		try {
			cmder.presetQuery(device, channelId, event -> {
				...
				msg.setId(uuid);
				msg.setKey(key);
				msg.setData(WVPResult.fail(String.format("获取设备预置位失败，错误码： %s, %s", event.statusCode, event.msg)));
				resultHolder.invokeResult(msg);
			});
