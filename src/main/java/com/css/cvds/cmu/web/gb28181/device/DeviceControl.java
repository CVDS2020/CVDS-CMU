...
public class DeviceControl {
    ...
    public WVPResult<?> teleBootApi(@PathVariable String deviceId) {
    ...
    public DeferredResult<ResponseEntity<String>> recordApi(@PathVariable String deviceId,
		...
		try {
			cmder.recordCmd(device, channelId, recordCmdStr, event -> {
				...
				msg.setId(uuid);
				msg.setKey(key);
				msg.setData(WVPResult.fail(String.format("开始/停止录像操作失败，错误码： %s, %s", event.statusCode, event.msg)));
				resultHolder.invokeAllResult(msg);
			});
		} catch (InvalidArgumentException | SipException | ParseException e) {
	...
	public DeferredResult<String> guardApi(@PathVariable String deviceId, String channelId, @PathVariable String guardCmdStr) {
		...
		try {
			cmder.guardCmd(device, guardCmdStr, event -> {
				...
				msg.setId(uuid);
				msg.setKey(key);
				msg.setData(WVPResult.fail(String.format("布防/撤防操作失败，错误码： %s, %s", event.statusCode, event.msg)));
	...
	public DeferredResult<ResponseEntity<String>> resetAlarmApi(@PathVariable String deviceId, String channelId,
		...
		try {
			cmder.alarmCmd(device, alarmMethod, alarmType, event -> {
				...
				msg.setId(uuid);
				msg.setKey(key);
				msg.setData(WVPResult.fail(String.format("报警复位操作失败，错误码： %s, %s", event.statusCode, event.msg)));
				resultHolder.invokeResult(msg);
			});
		} catch (InvalidArgumentException | SipException | ParseException e) {
	...
	public DeferredResult<String> homePositionApi(@PathVariable String deviceId,
		...
		try {
			cmder.homePositionCmd(device, channelId, enabled, resetTime, presetIndex, event -> {
				...
				msg.setId(uuid);
				msg.setKey(key);
				msg.setData(WVPResult.fail(String.format("看守位控制操作失败，错误码： %s, %s", event.statusCode, event.msg)));
				resultHolder.invokeResult(msg);
			});
		} catch (InvalidArgumentException | SipException | ParseException e) {
		...
		result.onTimeout(() -> {
			...
			msg.setId(uuid);
			msg.setKey(key);
			msg.setError("看守位控制操作超时, 设备未返回应答指令");
			resultHolder.invokeResult(msg);
		});
		resultHolder.put(key, uuid, result);
		return result;
	...
	@Parameter(name = "deviceId", description = "设备编号", required = true)
	@Parameter(name = "command", description = "控制命令：left, right, up, down, upleft, upright, downleft, downright, zoomin, zoomout, stop", required = true)
	@Parameter(name = "channelId", description = "通道ID")
	@Parameter(name = "speed", description = "控制命令")
	private WVPResult<?> list(String deviceId, String command,
							  @RequestParam(required = false)String channelId,
